---
layout: post
title: "Source Generators: The End of T4 Templates?"
date: 2024-02-25 12:00:00 +0000
tags: [source-generators, t4-templates, dotnet, csharp, code-generation, roslyn]
---
...
This leads to the question - **are source generators here to make T4 templates a thing from the past?** In this post we provide a high level overview and comparison between these tools.
...
### Example: Generating DTOs from Database Models
...
### Example: Generating DTOs with Source Generators

Just like in the T4 example above, we want to generate DTOs based on existing entities. In this example, any class that implements `IModelEntity` will have a corresponding DTO class generated for it at compile time. The generated DTOs will be placed in the *.DTOs namespace and will include properties that mirror those of the original model classes.
...
### When to Use T4 Templates

T4 Templates may still be the tool of choice for projects that:

- Require generating non-C# files (HTML, XML, JSON, etc.)
- Heavily rely on the specific templating capabilities and customizations that T4 provides
- Have existing infrastructure built around T4
- Teams are already familiar with T4 and have established workflows

### When to Use Source Generators

Source Generators are particularly well-suited for projects that:

- Benefit from performance optimizations at compile time
- Require generation of C# code based on the project's codebase itself
- Aim to leverage the latest .NET features with minimal impact on the build process
- Need strong IDE integration and debugging support
- Want to avoid runtime reflection in favor of compile-time code generation
...
The trend in modern .NET development is clearly moving towards source generators, with Microsoft investing heavily in this technology. However, T4 templates remain a viable option for specific use cases and will likely continue to be supported for existing projects.
...
- [T4Editor](https://github.com/Tim-Maes/T4Editor) by me
- [T4Language](https://github.com/bricelam/T4Language) by Brice Lambson
...
- [T4 Text Template Editors Comparison](https://medium.com/@david.zhao.blog/t4-text-template-editors-comparison-d58487db4ad3) by David Zhao
- [Unleashing the Power of Source Generators in .NET](https://reynders.co/blog/unleashing-the-power-of-source-generators-in-net/)

---

*Interested in learning more about code generation in .NET? Check out my other posts on .NET development and tooling!*
